...
_BilinearScale5_x86:
    ...
    %define stackSpace          0xc
    sub esp, stackSpace

   ;%define ratioDestXtoSrcX    [ebp - ]
    %define ratioDestYtoSrcY    [ebp - 0x10]
    %define srcY                [ebp - 0x14]
   ;%define srcX                [ebp - ]
   ;%define diffY               [ebp - ]
   ;%define diffX               [ebp - ]        ; now edi
    %define destY               [ebp - 0x18]
   ;%define destX               [ebp - ]
    ;%define srcIndex           [ebp - ]
    ;%define s0                  [ebp - ]
    ;%define s1                  [ebp - ]
    ...
    mov ecx, eax                    ; ecx (ratioDestXtoSrcX) = eax

    ; fixed ratioDestYtoSrcY = (((srcHeight - 1)<<FIXED) / destHeight);
    mov eax, srcHeight              ; eax = srcHeight
    mov ebx, destHeight             ; ebx = destHeight
    sub eax, 1                      ; eax -= 1
    shl eax, FIXED                  ; eax <<= FIXED
    mov edx, 0                      ; setup for div instruction
    div ebx                         ; eax = eax / ebx; edx = eax % ecx
...
.LoopDestX:
    ...
    mov eax, edi
    shl eax, FIXED                  ; eax <<= FIXED
    mov ebx, ecx                    ; ebx = ecx (ratioDestXtoSrcX)
    mul ebx                         ; eax *= ebx
    ...
    ;mov eax, destX                  ; eax = destX
    mov eax, edi                    ; eax = edi (destX)
    shl eax, FIXED                  ; eax (destX) <<= FIXED
    mov ebx, ecx                    ; ebx = ecx (ratioDestXtoSrcX)
    mul ebx                         ; eax (destX << FIXED) *= ebx (ratioDestXtoSrcX)
    shr eax, FIXED_2X               ; eax (srcX <-- ((destX<<FIXED)*ratioDestXtoSrcX)) >>= FIXED_2X
    ;mov dword srcX, eax             ; srcX = eax
    shl eax, 2                      ; eax (srcX) *= 4
    mov ebx, eax                    ; ebx = eax (srcX * 4)
    ...
    add eax, ebx                    ; eax (srcY -> srcIndex) += ecx (srcX)
    mov ebx, src                    ; ebx = src
    ...
    ; eax, ebx, edx: 32-bit fixed-point -> 8-bit values
    shr eax, FIXED
    shr ebx, FIXED
    shr edx, FIXED
    and eax, 0xFF
    ...
    mov [ebx], eax          ; *ebx (dest) = edx (colorFinal)
    add ebx, 4              ; ebx += 4
